...
def copy_paste_middle(src, dst, shape):
    ...
    print src.shape
    src_mid = src[(src.shape[0]/2-h):(src.shape[0]/2+h),(src.shape[1]/2-w):(src.shape[1]/2+w)]
    tmp = np.copy(dst)
    tmp[(tmp.shape[0]/2-h):(tmp.shape[0]/2+h),(tmp.shape[1]/2-w):(tmp.shape[1]/2+w)] = src_mid
...
def image_stats(image):
    ...
    """
    stats = [np.min(image), np.max(image), np.mean(image), np.std(image)]
    return stats
    raise NotImplementedError
